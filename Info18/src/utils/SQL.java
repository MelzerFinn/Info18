...
public class SQL {
	 ...
	 * @return Boolean to indicate error (false) or success (true).
	 */
	public static boolean save(Point3D... points) {
		String s = "";
		...
		try {
			Statement stmt = conn.createStatement();
			stmt.executeUpdate("INSERT INTO `current3D` (`ID`, `JsonString`, `FETCHED`) VALUES (NULL, '"+s+"', '0');");
		} catch (Exception e) {
			handleError(e);
			return false;
		}
	...
	public static boolean init() {
		try {
			...
			stmt = conn.createStatement();
			stmt.executeUpdate(
					"CREATE TABLE `data`.`current3D` ( `ID` INT NOT NULL AUTO_INCREMENT ,  `JsonString` LONGTEXT NOT NULL ,  `FETCHED` BOOLEAN NOT NULL DEFAULT FALSE ,    PRIMARY KEY  (`ID`)) ENGINE = InnoDB;");
			stmt = conn.createStatement();
			stmt.executeUpdate(
